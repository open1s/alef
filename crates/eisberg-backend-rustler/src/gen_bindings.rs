...
impl Backend for RustlerBackend {
    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        for error in &api.errors {
            builder.add_item(&eisberg_codegen::error_gen::gen_rustler_error_converter(
                error,
                &core_import,
            ));
        }
        ...
        let _adapter_bodies = eisberg_adapters::build_adapter_bodies(config, Language::Elixir)?;
...
fn gen_rustler_config_impl(typ: &TypeDef, mapper: &RustlerMapper) -> String {
    ...
    let config_method = eisberg_codegen::config_gen::gen_rustler_kwargs_constructor(typ, &map_fn);
...
fn gen_rustler_unimplemented_body(return_type: &eisberg_core::ir::TypeRef, fn_name: &str, has_error: bool) -> String {
...
fn map_return_type(ty: &eisberg_core::ir::TypeRef, mapper: &RustlerMapper, opaque_types: &AHashSet<String>) -> String {
    use eisberg_core::ir::TypeRef;
    match ty {
