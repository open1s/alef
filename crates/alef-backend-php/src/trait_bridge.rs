...
/// Generate a PHP static method for an options-field bridge.
///
/// When `bind_via = "options_field"`, the visitor lives on the options struct
/// (`options.visitor`).  ext-php-rs cannot read a raw-pointer field from a
/// PHP class struct, so the generated Rust function takes the visitor as an
/// *extra* parameter (`{field_name}_obj: Option<&mut ZendObject>`).  The PHP
/// facade is responsible for passing `$options->{field_name}` as that extra arg.
#[allow(clippy::too_many_arguments)]
pub fn gen_bridge_field_function(
    func: &alef_core::ir::FunctionDef,
    field_match: &BridgeFieldMatch<'_>,
    ...
    let struct_name = format!("Php{}Bridge", field_match.bridge.trait_name);
    let handle_path = format!("{core_import}::visitor::VisitorHandle");
    let field_name = &field_match.field_name;
    let extra_param = format!("{field_name}_obj");

    // Build parameter list (all original params, then the extra visitor obj).
    ...
    // Bridge wrapping from the extra param.
    let bridge_wrap = format!(
        "let {extra_param} = {extra_param}.map(|v| {{\n        \
         let bridge = {struct_name}::new(v);\n        \
         std::rc::Rc::new(std::cell::RefCell::new(bridge)) as {handle_path}\n    \
         }});"
    );

    // Serde round-trip for Named (non-opaque) options param so we get the core type.
    let opts_param_name = &func.params[field_match.param_index].name;
    let opts_is_optional = field_match.param_is_optional;
    ...
    // Attach the visitor to the options core struct.
    let visitor_attach = if opts_is_optional {
        format!(
            "if let (Some(ref mut opts), Some(v)) = ({opts_param_name}_core.as_mut(), {extra_param}) {{\n        \
             opts.{field_name} = Some(v);\n    \
             }}\n    "
        )
    } else {
        format!(
            "if let Some(v) = {extra_param} {{\n        \
             {opts_param_name}_core.{field_name} = Some(v);\n    \
             }}\n    "
        )
    };
