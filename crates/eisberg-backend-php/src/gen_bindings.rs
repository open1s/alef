...
use ahash::AHashSet;
use eisberg_codegen::builder::{ImplBuilder, RustFileBuilder};
use eisberg_codegen::conversions::ConversionConfig;
use eisberg_codegen::generators::{self, AsyncPattern, RustBindingConfig};
use eisberg_codegen::shared::{self, constructor_parts, partition_methods};
use eisberg_codegen::type_mapper::TypeMapper;
use eisberg_core::backend::{Backend, Capabilities, GeneratedFile};
use eisberg_core::config::{Language, SkifConfig, detect_serde_available, resolve_output_dir};
use eisberg_core::ir::{ApiSurface, EnumDef, FunctionDef, MethodDef, PrimitiveType, TypeDef, TypeRef};
...
impl Backend for PhpBackend {
    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        let convertible = eisberg_codegen::conversions::convertible_types(api);
        ...
        for typ in &api.types {
            ...
            if !enum_tainted.contains(&typ.name)
                && eisberg_codegen::conversions::can_generate_conversion(typ, &convertible)
            {
                builder.add_item(&eisberg_codegen::conversions::gen_from_binding_to_core_cfg(
                    typ,
                    &core_import,
                    &php_conv_config,
                ));
            ...
            if eisberg_codegen::conversions::can_generate_conversion(typ, &core_to_binding) {
                builder.add_item(&eisberg_codegen::conversions::gen_from_core_to_binding_cfg(
                    typ,
                    &core_import,
                    &opaque_types,
                    &php_conv_config,
        ...
        for error in &api.errors {
            builder.add_item(&eisberg_codegen::error_gen::gen_php_error_converter(
                error,
                &core_import,
            ));
        }

        let _adapter_bodies = eisberg_adapters::build_adapter_bodies(config, Language::Php)?;
    ...
    fn generate_public_api(&self, api: &ApiSurface, config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        content.push_str("// This file is auto-generated by eisberg. DO NOT EDIT.\n");
...
fn type_ref_has_named(ty: &eisberg_core::ir::TypeRef) -> bool {
...
fn gen_php_unimplemented_body(return_type: &eisberg_core::ir::TypeRef, fn_name: &str, has_error: bool) -> String {
    use eisberg_core::ir::TypeRef;
...
fn gen_serde_bridge_from(typ: &TypeDef, core_import: &str) -> String {
    ...
    format!(
        "impl From<{}> for {} {{\n    \
         fn from(val: {}) -> Self {{\n        \
         let json = serde_json::to_string(&val).expect(\"eisberg: serialize binding type\");\n        \
         serde_json::from_str(&json).expect(\"eisberg: deserialize to core type\")\n    \
         }}\n\
         }}",
...
fn references_named_type(ty: &eisberg_core::ir::TypeRef, names: &AHashSet<String>) -> bool {
    use eisberg_core::ir::TypeRef;
    match ty {
...
fn has_enum_named_field(typ: &eisberg_core::ir::TypeDef, enum_names: &AHashSet<String>) -> bool {
    fn type_ref_has_enum_named(ty: &eisberg_core::ir::TypeRef, enum_names: &AHashSet<String>) -> bool {
        use eisberg_core::ir::TypeRef;
        match ty {
...
fn gen_php_call_args(params: &[eisberg_core::ir::ParamDef], opaque_types: &AHashSet<String>) -> String {
    params
        .iter()
        .map(|p| match &p.ty {
            TypeRef::Named(name) if opaque_types.contains(name.as_str()) => {
...
fn gen_enum_tainted_from_binding_to_core(
    ...
    for field in &typ.fields {
        ...
        if field.sanitized {
            ...
            let conversion =
                eisberg_codegen::conversions::field_conversion_to_core_cfg(name, &field.ty, field.optional, config);
...
fn gen_string_to_enum_expr(
    ...
    let core_enum_path = eisberg_codegen::conversions::core_enum_path(enum_def, core_import);
    ...
    fn variant_expr(core_path: &str, variant: &eisberg_core::ir::EnumVariant) -> String {
        if variant.fields.is_empty() {
            format!("{core_path}::{}", variant.name)
        } else if eisberg_codegen::conversions::is_tuple_variant(&variant.fields) {
