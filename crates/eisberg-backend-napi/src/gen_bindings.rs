...
use eisberg_codegen::naming::to_node_name;
use eisberg_codegen::shared::{can_auto_delegate, function_params, partition_methods};
...
impl Backend for NapiBackend {
    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        for typ in &api.types {
            if typ.is_opaque {
                builder.add_item(&eisberg_codegen::generators::gen_opaque_struct_prefixed(
                    typ, &cfg, "Js",
                ));
                ...
                if typ.has_default {
                    builder.add_item(&eisberg_codegen::generators::gen_struct_default_impl(typ, "Js"));
                }
            }
        }
        ...
        for func in &api.functions {
            ...
            let has_opaque_param = func.params.iter().any(|p| {
                if let eisberg_core::ir::TypeRef::Named(n) = &p.ty {
        ...
        let napi_conv_config = eisberg_codegen::conversions::ConversionConfig {
            type_name_prefix: "Js",
        ...
        for e in &api.enums {
            if eisberg_codegen::conversions::can_generate_enum_conversion(e) {
                builder.add_item(&eisberg_codegen::conversions::gen_enum_from_binding_to_core_cfg(
                    e,
                    &core_import,
                    &napi_conv_config,
                ));
            ...
            if eisberg_codegen::conversions::can_generate_enum_conversion_from_core(e) {
                builder.add_item(&eisberg_codegen::conversions::gen_enum_from_core_to_binding_cfg(
                    e,
                    &core_import,
                    &napi_conv_config,
                ));
        ...
        for error in &api.errors {
            builder.add_item(&eisberg_codegen::error_gen::gen_napi_error_types(error));
            builder.add_item(&eisberg_codegen::error_gen::gen_napi_error_converter(
                error,
                &core_import,
            ));
        }
        ...
        let _adapter_bodies = eisberg_adapters::build_adapter_bodies(config, Language::Node)?;
    ...
    fn generate_public_api(&self, api: &ApiSurface, _config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
...
fn gen_opaque_instance_method(
    ...
    let opaque_can_delegate = !method.sanitized
        ...
        && method
            .params
            .iter()
            .all(|p| !p.sanitized && eisberg_codegen::shared::is_delegatable_param(&p.ty, opaque_types))
        && eisberg_codegen::shared::is_opaque_delegatable_type(&method.return_type);
...
fn gen_function(
    ...
    let can_delegate_fn = eisberg_codegen::shared::can_auto_delegate_function(func, opaque_types);
...
fn needs_napi_cast(p: &eisberg_core::ir::PrimitiveType) -> bool {
    matches!(
        p,
        eisberg_core::ir::PrimitiveType::U64
            | eisberg_core::ir::PrimitiveType::Usize
            | eisberg_core::ir::PrimitiveType::Isize
    )
}

fn core_prim_str(p: &eisberg_core::ir::PrimitiveType) -> &'static str {
    match p {
        eisberg_core::ir::PrimitiveType::U64 => "u64",
        eisberg_core::ir::PrimitiveType::Usize => "usize",
        eisberg_core::ir::PrimitiveType::Isize => "isize",
        _ => unreachable!(),
    }
}
