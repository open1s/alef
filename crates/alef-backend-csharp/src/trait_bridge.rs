...
pub fn gen_trait_bridges_file(
    ...
    writeln!(out, "// This file bridges managed C# trait implementations to the native FFI layer").ok();
    writeln!(out).ok();
...
fn gen_single_trait_bridge(
    ...
    writeln!(out, "/// Bridge interface for {} trait implementation via native FFI", trait_pascal).ok();
    writeln!(out, "/// </summary>").ok();
    ...
    // --- Bridge Class ---
    writeln!(out, "/// <summary>").ok();
    writeln!(out, "/// Manages the FFI vtable and delegates for a {} implementation", trait_pascal).ok();
    writeln!(out, "/// </summary>").ok();
    ...
    if has_super_trait {
        ...
        writeln!(out, "    private delegate int NameFn(IntPtr userData, out IntPtr outName);").ok();
        writeln!(out).ok();

        writeln!(out, "    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]").ok();
        writeln!(out, "    private delegate int VersionFn(IntPtr userData, out IntPtr outVersion);").ok();
        writeln!(out).ok();

        writeln!(out, "    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]").ok();
        writeln!(out, "    private delegate int InitializeFn(IntPtr userData, out IntPtr outError);").ok();
        writeln!(out).ok();

        writeln!(out, "    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]").ok();
        writeln!(out, "    private delegate int ShutdownFn(IntPtr userData, out IntPtr outError);").ok();
        writeln!(out).ok();
    }
    ...
    writeln!(out, "        _impl = impl ?? throw new ArgumentNullException(nameof(impl));").ok();
    ...
    writeln!(out, "        // Allocate unmanaged vtable struct (array of function pointers)").ok();
    writeln!(
        out,
    ...
    if has_super_trait {
        ...
        writeln!(out, "    private int VersionFnCallback(IntPtr userData, out IntPtr outVersion) {{").ok();
        writeln!(out, "        try {{").ok();
        ...
        writeln!(out, "    private int InitializeFnCallback(IntPtr userData, out IntPtr outError) {{").ok();
        writeln!(out, "        try {{").ok();
        ...
        writeln!(out, "    private int ShutdownFnCallback(IntPtr userData, out IntPtr outError) {{").ok();
        writeln!(out, "        try {{").ok();
    ...
    for method in &trait_def.methods {
        ...
        if method.return_type == TypeRef::Unit {
            ...
            writeln!(out, "            outResult = Marshal.StringToCoTaskMemUTF8(ToJsonString(result));").ok();
        }
    ...
    // --- Registry Class ---
    writeln!(out, "/// <summary>Static helpers for registering trait implementations</summary>").ok();
    writeln!(out, "public static class {}Registry {{", trait_pascal).ok();
    writeln!(out).ok();

    writeln!(out, "    private static readonly ConcurrentDictionary<string, {}Bridge> _bridges =",
        trait_pascal
    ).ok();
    writeln!(out, "        new ConcurrentDictionary<string, {}Bridge>();", trait_pascal).ok();
    writeln!(out).ok();

    writeln!(out, "    /// <summary>Register a {} implementation</summary>", trait_pascal).ok();
    ...
    writeln!(out).ok();
    writeln!(out, "        try {{").ok();
    writeln!(out, "            var userDataHandle = GCHandle.Alloc(bridge, GCHandleType.Normal);").ok();
    ...
    writeln!(out, "    /// <summary>Unregister a {} implementation</summary>", trait_pascal).ok();
    writeln!(out, "    public static void Unregister(string name) {{").ok();
    writeln!(out, "        if (string.IsNullOrEmpty(name))").ok();
    writeln!(out, "            throw new ArgumentException(\"Name cannot be empty\", nameof(name));").ok();
    writeln!(out).ok();
    writeln!(
        out,
