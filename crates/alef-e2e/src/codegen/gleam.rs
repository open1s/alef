...
impl E2eCodegen for GleamE2eCodegen {
    fn generate(
        ...
        // When no fixture-driven tests were generated, emit a minimal smoke test.
        if !any_tests {
            let smoke = concat!(
...
fn render_test_file(
    ...
    if has_http_fixtures {
        let _ = writeln!(out, "import gleam_httpc");
        ...
        let _ = writeln!(out, "import gleam/os");
    }
...
fn render_http_test_case(out: &mut String, fixture: &Fixture) {
    ...
    let _ = writeln!(out, "  let base_url = case os.get_env(\"MOCK_SERVER_URL\") {{");
    ...
    // Send request.
    let _ = writeln!(out, "  let assert Ok(resp) = gleam_httpc.send(req)");
    ...
    for (name, value) in &expected.headers {
        ...
        let _ = writeln!(
            out,
            "  resp.headers
    |> list.find(fn(h) {{ h.0 == \"{escaped_name}\" }})
    |> option.is_some()
    |> should.be_true()"
        );
    }
