...
/// Convert a `serde_json::Value` object to an Elixir struct literal.
///
/// Rustler's `NifStruct` decoder requires a proper Elixir struct (with `__struct__` and
/// atom keys) — plain string-keyed maps are rejected and cause a NIF panic.
/// This function emits `%ModulePath.StructName{key: val, ...}` syntax.
///
/// `atom_list_fields` names fields whose array elements are Elixir atoms (Rustler
/// `NifUnitEnum`), so string values are rendered as atoms (`:image` not `"image"`).
fn json_to_elixir_struct(
    ...
    match value {
        serde_json::Value::Object(map) if !map.is_empty() => {
            let entries: Vec<String> = map
                ...
                .map(|(k, v)| {
                    let snake_key = k.to_snake_case();
                    let elixir_val = if atom_list_fields.contains(&snake_key) {
                        json_to_elixir_atom_list(v)
                    } else {
                        json_to_elixir(v)
                    };
        ...
        other => json_to_elixir(other),
    }
}

/// Convert a JSON array of strings to an Elixir list of atoms.
///
/// Used for `NifUnitEnum` fields where the Rust type is `Vec<SomeEnum>` and the
/// Elixir side must pass atoms (`:image`) not strings (`"image"`).
fn json_to_elixir_atom_list(value: &serde_json::Value) -> String {
    match value {
        serde_json::Value::Array(arr) => {
            let items: Vec<String> = arr
                .iter()
                .map(|v| match v {
                    serde_json::Value::String(s) => format!(":{}", s.to_snake_case()),
                    other => json_to_elixir(other),
                })
                .collect();
            format!("[{}]", items.join(", "))
