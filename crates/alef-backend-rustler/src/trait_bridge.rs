...
/// Generate an async visitor NIF for an `options_field` bridge.
///
/// When `bind_via = "options_field"`, the visitor does not appear as a positional
/// IR parameter. Instead it lives on a field of the options struct. The regular
/// NIF (`convert`) is generated by the standard code path; this function only
/// emits the `convert_with_visitor` variant that appends a synthetic
/// `visitor: Option<rustler::Term<'_>>` parameter.
///
/// The Elixir public-API layer extracts `:visitor` from the options map and
/// dispatches to either the regular NIF or this async variant.
...
pub fn gen_bridge_field_function(
    ...
    let bridge_cfg = bridge_match.bridge;
    let struct_name = format!("Elixir{}Bridge", bridge_cfg.trait_name);
    let handle_path = format!("{core_import}::visitor::VisitorHandle");
    let field_name = bridge_match.field_name.as_str();
    let options_type_name = bridge_match.options_type.as_str();

    // Build NIF signature: all original IR params + synthetic visitor at the end.
    let mut sig_parts = vec!["env: rustler::Env<'_>".to_string()];
    let mut deser_lines: Vec<String> = Vec::new();
    let mut clone_stmts: Vec<String> = Vec::new();
    // call_args maps to the core function arguments (using the options type WITH visitor attached).
    ...
    for p in &func.params {
        if let TypeRef::Named(n) = &p.ty {
            if default_types.contains(n) {
                ...
                // For the options type carrying the bridge field, we build a modified version.
                if n == options_type_name {
                    // Will be replaced below with options_with_visitor.
                    call_args.push(format!("{}_with_visitor", p.name));
                } else if p.optional {
                    call_args.push(format!("{}_core", p.name));
                } else if p.is_ref {
                    call_args.push(format!("{}_core.as_ref().unwrap_or(&Default::default())", p.name));
                } else {
                    call_args.push(format!("{}_core.unwrap_or_default()", p.name));
                }
                continue;
            }
        }
        let mapped = mapper.map_type(&p.ty);
        if p.optional {
            sig_parts.push(format!("{}: Option<{}>", p.name, mapped));
        } else {
            sig_parts.push(format!("{}: {mapped}", p.name));
        }
        match &p.ty {
            TypeRef::String | TypeRef::Char => {
                clone_stmts.push(format!("let {0} = {0}.clone();", p.name));
                if p.is_ref {
            ...
            TypeRef::Named(n) if opaque_types.contains(n) => {
                if p.optional {
                    call_args.push(format!("{}.as_ref().map(|v| &v.inner)", p.name));
                } else {
                    call_args.push(format!("&{}.inner", p.name));
                }
            }
            _ => {
                call_args.push(p.name.clone());
    ...
    // Synthetic visitor parameter
    sig_parts.push(format!("{field_name}: Option<rustler::Term<'_>>"));

    let with_params_str = sig_parts.join(", ");

    // Find the options param name for the "with_visitor" binding.
    let options_param = func
        .params
        .iter()
        .find(|p| match &p.ty {
            TypeRef::Named(n) => n == options_type_name,
            _ => false,
        })
        .map(|p| p.name.as_str())
        .unwrap_or("options");
    ...
    let func_name = &func.name;
    let deser_str = if deser_lines.is_empty() {
        String::new()
    } else {
        format!("{}\n    ", deser_lines.join("\n    "))
    };
    let clone_str = if clone_stmts.is_empty() {
        String::new()
    } else {
        format!("{}\n    ", clone_stmts.join("\n    "))
    };
    ...
    .ok();
    writeln!(out, "    let pid = env.pid();").ok();
    writeln!(out, "    {deser_str}").ok();

    // Build options_with_visitor by cloning the deserialized options and attaching the bridge.
    writeln!(out, "    let visitor_owned_env = rustler::OwnedEnv::new();").ok();
    writeln!(
        out,
        "    let visitor_saved = visitor_owned_env.save({field_name}.expect(\"visitor present in _with_visitor\"));"
    )
    .ok();
    writeln!(
        out,
        "    let {options_param}_base = {options_param}_core.unwrap_or_default();"
    )
    .ok();
    writeln!(out, "    {clone_str}").ok();
    ...
    writeln!(
        out,
        "        let visitor_handle: Option<{handle_path}> = Some(std::rc::Rc::new(std::cell::RefCell::new(bridge)) as {handle_path});"
    )
    .ok();
    writeln!(
        out,
        "        let mut {options_param}_with_visitor = {options_param}_base;"
    )
    .ok();
    writeln!(
        out,
        "        {options_param}_with_visitor.{field_name} = visitor_handle;"
    )
    .ok();

    writeln!(out, "        let mut result_env = rustler::OwnedEnv::new();").ok();
    writeln!(out, "        let _ = result_env.send_and_clear(&pid, |env| {{").ok();
    writeln!(out, "            match {core_fn_path}({call_args_str}) {{").ok();
    writeln!(out, "                Ok(val) => {{").ok();
    writeln!(out, "                    let result: ConversionResult = val.into();").ok();
    writeln!(
        out,
