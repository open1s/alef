...
fn gen_instance_method(
    ...
    let body = if can_delegate {
        ...
        let field_conversions = if needs_mut_receiver {
            generators::gen_lossy_binding_to_core_fields_mut(typ, core_import, false, opaque_types)
        } else {
            generators::gen_lossy_binding_to_core_fields(typ, core_import, false, opaque_types)
        };
