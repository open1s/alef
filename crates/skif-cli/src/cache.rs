...
/// Read cached IR.
pub fn read_cached_ir() -> anyhow::Result<skif_core::ir::ApiSurface> {
    let ir_path = Path::new(CACHE_DIR).join("ir.json");
...
/// Write IR to cache.
pub fn write_ir_cache(api: &skif_core::ir::ApiSurface, source_hash: &str) -> anyhow::Result<()> {
