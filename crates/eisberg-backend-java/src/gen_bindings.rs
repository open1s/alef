...
impl Backend for JavaBackend {
    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &SkifConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        for error in &api.errors {
            for (class_name, content) in eisberg_codegen::error_gen::gen_java_error_types(error, &package) {
                files.push(GeneratedFile {
...
fn gen_builder_class(package: &str, typ: &TypeDef) -> String {
    ...
    if typ
        .fields
        .iter()
        .any(|f| f.optional || matches!(f.ty, TypeRef::Vec(_) | TypeRef::Map(_, _)))
    {
        writeln!(out, "import java.util.List;").ok();
        writeln!(out, "import java.util.Map;").ok();
        writeln!(out, "import java.util.Optional;").ok();
    ...
    writeln!(out, "public class {}Builder {{", typ.name).ok();
    writeln!(out).ok();

    // Generate field declarations with defaults
    for field in &typ.fields {
        let field_name = to_java_name(&field.name);
        ...
        writeln!(out, "    private {} {} = {};", field_type, field_name, default_value).ok();
    }
    ...
    for field in &typ.fields {
        ...
        .ok();
        writeln!(out, "        this.{} = value;", field_name).ok();
        writeln!(out, "        return this;").ok();
        writeln!(out, "    }}").ok();
        writeln!(out).ok();
    }

    // Generate build() method
    writeln!(out, "    public {} build() {{", typ.name).ok();
    writeln!(out, "        return new {}(", typ.name).ok();
    ...
    for (i, field) in non_tuple_fields.iter().enumerate() {
        ...
        writeln!(out, "            {}{}", field_name, comma).ok();
    }
    writeln!(out, "        );").ok();
    writeln!(out, "    }}").ok();
