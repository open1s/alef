...
fn gen_enum_class(package: &str, enum_def: &EnumDef) -> String {
    ...
    for (i, variant) in enum_def.variants.iter().enumerate() {
        ...
        writeln!(out, "    {}(\"{}\")", variant.name, json_name).ok();
        // Add comma/semicolon after the constructor call
        write!(out, "{}", comma).ok();
        writeln!(out).ok();
    }
    ...
    writeln!(out).ok();
    writeln!(out, "    {}(String value) {{ this.value = value; }}", enum_def.name).ok();
    writeln!(out).ok();
    writeln!(out, "    @JsonValue").ok();
    writeln!(out, "    public String getValue() {{ return value; }}").ok();
    writeln!(out).ok();
    ...
    writeln!(out, "            if (e.value.equalsIgnoreCase(value)) return e;").ok();
    writeln!(out, "        }}").ok();
    writeln!(
        out,
...
fn gen_java_tagged_union(package: &str, enum_def: &EnumDef) -> String {
    ...
    writeln!(out, "package {};", package).ok();
    writeln!(out).ok();
    writeln!(out, "import com.fasterxml.jackson.annotation.*;").ok();

    // Check if any field types need list/map/optional imports
    let needs_list = enum_def
        .variants
        .iter()
        .any(|v| v.fields.iter().any(|f| matches!(&f.ty, TypeRef::Vec(_))));
    let needs_map = enum_def
        .variants
        .iter()
        .any(|v| v.fields.iter().any(|f| matches!(&f.ty, TypeRef::Map(_, _))));
    let needs_optional = enum_def.variants.iter().any(|v| v.fields.iter().any(|f| f.optional));
    if needs_list {
        writeln!(out, "import java.util.List;").ok();
    }
    if needs_map {
